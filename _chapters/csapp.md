...
这个 Lab 里，你不仅要看汇编，你还要手写汇编，甚至要手搓 CPU 硬件连线！

直接编译会失败，原因是这套老代码还依赖旧 GCC 的 `-fcommon` 语义。而该语义在 20 年左右默认从 `-fcommon` 变成 `-fno-common`。此外，还有些 GUI 路线的问题等等，总之在还没开始的起步阶段就非常折磨人了。整个 Project 的发射可以参考以下脚本修改：

```bash
cd ./src/archlab-handout/sim/misc
make clean
...
make copy.yo
./yis copy.yo
make -C ../y86-code asumi.yo
cd ../seq
make clean
make ssim VERSION=full GUIMODE= CFLAGS='-Wall -O2 -fcommon'
./ssim -t ../y86-code/asumi.yo
cd ../ptest
make SIM=../seq/ssim TFLAGS=-i
cd ../pipe
make clean
make psim VERSION=full GUIMODE= CFLAGS='-Wall -O2 -fcommon'
./correctness.pl -p -f ncopy.ys
./benchmark.pl -f ncopy.ys
cd ../ptest
...
先在 `misc` 下创建 `sum.ys`，`rsum.ys`，`copy.ys`。

> `.ys` 是什么
> 
> `.ys` 是 `Y86-64 assembly source`，也就是 `Y86-64` 汇编源文件。
> 但我完全不知道怎么写 Y86？先不要急着写，先看这两个文件：`sim/misc/examples.c` 和 `sim/y86-code/asum.ys`。前者告诉你“语义是什么”，后者告诉你“Y86-64 程序长什么样”。

我们在 Part A 需要实现的是就是将 c 语言翻译为 y86，翻译的内容是：
- `sum_list`：给一个链表头指针，遍历链表，把所有节点的 `val` 加起来
- `rsum_list`：同样是链表求和，但用递归
- `copy_block`：把一个数组从 `src` 拷到 `dest`，同时返回所有源元素的异或值

这一部分就是纯粹的增加对 y86 汇编语言的熟练度。

### HCL

Part B 要给 SEQ 这个单周期 Y86-64 处理器，加上一条新指令 `iaddq`。

我们要改的是 `sim/seq/seq-full.hcl` 中 SEQ 处理器的控制逻辑描述。`iaddq` 的语义大概就是把一个立即数加到寄存器里。我们实现它类似一个数据通路控制设计问题。

大多数其实还是看源码写源码的累活，这个 Part B 的话，其实就是注意这八个问题：
...
就可以完成了，看着简单，只是改八行代码就能通过，但其实主要还是要看懂这些代码的内容语法，重点就是先看懂它。

### Pipeline

终极折磨。你要修改 CPU 的流水线连线（Pipeline），并通过**循环展开**等极度恶心的纯汇编技巧，把一个拷贝数组的汇编代码优化到极限的 CPE。

Part C 主要有两件事：
1. 让 PIPE 这台流水线处理器也支持 iaddq（`/sim/pipe/pipe-full.hcl`），和 Part B 类似，但因为 PIPE 是流水线，所以除了指令语义是什么，还要考虑流水段、forwarding、条件码、hazard 等等。不过对 iaddq 本身来说，改动还是相对局部的，本质仍是类似 Part B。
2. 优化 ncopy.ys 的性能（`sim/pipe/ncopy.ys`），这个函数的语义在 `sim/pipe/ncopy.c` 里已经给了。所以 Part C 不是再造功能，而是在功能必须保持正确的情况下，同时尽量降低流水线执行它时的 CPE。

功能实现了，但问题是默认实现的性能非常惨，Average CPE 只有 15.18，Score 也只有 0.0/60.0。

其实这里明显要做循环展开等技巧优化，至少循环展开是第一个就能想到的非常明显的优化。但看着不断运行脚本而 Score 几乎稳如泰山甚至可能出现不减少反增时，我真的不想为了压 CPE 去写很恶心的 Y86 优化了，汇编语言再见吧赶紧往下。对不起了原谅我吧。🥺

## Project 4: Cachelab

欢迎回到高级语言的世界！

经历了位运算的折磨、CISC 屎山的恶心、Y86 流水线的毒打后，现在终于能关上汇编语言的大门了。

P4 要手搓的是非常有意思且极为核心的底层组件 Cache（缓存）的简单模拟。不仅仅是 CPU 的 L1 Cache，放眼整个计算机科学领域，到处都能看到它的影子：OS 的 Page Cache、文件系统的缓存、网络协议栈的缓冲区、编译器的符号表缓冲，甚至 MLSys 中的数据预取队列。Cache 的核心哲学就是“欺骗”：将慢速的底层存储抽象为快速的内存，使得上层应用产生一种“数据全在内存中”的错觉，完全无需关心底层存储的换入换出细节。
