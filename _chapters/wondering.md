...
Within this abstract realm, the physical quantities we define are no longer diffuse fields but precise mathematical variables. We define **Current** as the time rate of flow of electric charge ($i = dq/dt$), representing the "through" variable; and **Voltage** as the energy required to move a unit of charge ($v = dw/dq$), representing the "across" potential difference. These two variables do not exist in isolation; their product yields **Power**, the instantaneous rate of energy exchange. To mathematically distinguish whether energy is being "absorbed" or "delivered" by an element, we introduce the **Passive Sign Convention**. This strict sign notation serves as the first "grammatical rule" of circuit analysis, ensuring that every equation we write accurately reflects the physical essence of energy conservation.

With the language defined, we require the building blocks to construct our circuits. In this abstract model, we introduce **Ideal Circuit Elements**. On one side are **Independent Sources**, serving as energy providers that rigidly dictate potential difference or flow rate. On the other lies the **Resistor**, representing the purest **Linear Constraint** (**Ohm's Law**, $v=iR$) and the dissipation of energy. Moreover, to model the behavior of complex devices like amplifiers and transistors, we introduce **Dependent Sources**, whose output is controlled by variables elsewhere in the circuit, which grants the circuit the ability to "sense" and "respond" to its own state, forming the mathematical foundation of modern electronics. These simple, idealized linear blocks form the starting point from which we analyze any complex DC network.

With the fundamental blocks defined, when we interconnect these elements into a network, they must obey the supreme constitution governing circuit behavior: **Kirchhoff's Laws**. KCL embodies the conservation of charge, while KVL embodies the conservation of energy. Resting on these pillars, we derive the intuitive rules of **Series and Parallel circuits**, as well as **Voltage and Current Division**. However, not all circuits are compliant. Faced with stubborn topologies that defy simple series-parallel reduction, we wield the **Delta-Wye Transformation** ($\Delta$-Y Transformation), a tool that resolves complexity by restructuring the topology itself, and the **Wheatstone Bridge**, which demonstrates how to measure the unknown through the precise balance of potentials. These theorems empower us to reduce complex networks into simpler forms, allowing us to perceive the relationship between the local and the global through elegant algebraic operations.

With the constitutions defined, we transcend ad-hoc simplification and establish systematic methodologies for universal analysis. We introduce the **Node-Voltage** and **Mesh-Current Methods**, algorithmic approaches that transform physical topology into standardized linear algebraic systems ($Ax=b$), ensuring that a solution can be found for any circuit, no matter how intricate. Yet, the most profound insight lies in "Equivalence." **Thevenin’s** and **Norton’s Theorems** teach us the art of the **"Black Box"**—demonstrating that any complex linear network, viewed from its terminals, is indistinguishable from a single source and resistor. This shift in perspective, combined with the **Principle of Superposition** for decomposing multiple excitations and the **Maximum Power Transfer Theorem** for optimizing efficiency, completely liberates our ability to analyze complex systems.

With the methodologies defined, we are now equipped to integrate the **Operational Amplifier** (Op Amp)—the heart of modern analog electronics—into our abstract model. Although physically composed of dozens of transistors, through the lens of circuit analysis, we elegantly abstract it into the **Ideal Op Amp Model**. From this model emerge two immensely powerful constraints: the **"Virtual Short"** and the **"Virtual Open"**. These rules, combined with the ingenious mechanism of **Negative Feedback**, empower us to construct precise circuits capable of performing addition, subtraction, and complex mathematical operations without regard for the device's internal physics, truly realizing the vision of "computing" via circuitry.

While Op Amps endow circuits with algebraic computational power, our model remains shackled to the "instantaneous"—the circuit responds only to the present, possessing no memory. To break this limitation, we integrate the **Inductor** and the **Capacitor** into our abstract framework. Unlike resistors that dissipate energy, these are **Storage Elements**: the capacitor stores potential energy within an electric field, while the inductor stores kinetic energy within a magnetic field. Their introduction fundamentally transforms the mathematical nature of circuits—the relationship between voltage and current evolves from simple proportionality into dynamic **Differentiation and Integration** ($v = L di/dt$ and $i = C dv/dt$). This not only grants circuits the capacity to process time-varying signals but also, through the concept of **Mutual Inductance**, enables the coupling of energy and information across space without physical contact, laying the foundation for transformers and wireless transmission.

With storage elements granting our circuits the power of memory, we bid farewell to static DC analysis and formally enter the dynamic **Time Domain**. Here, we focus on fundamental **First-Order Circuits** (RL & RC). When a switch flips, disrupting equilibrium, the circuit does not jump instantly but evolves smoothly according to first-order differential equations, exhibiting an exponential transition—a phenomenon known as **Transient Response**. We study the **Natural Response** to observe how stored energy decays, and the **Step Response** to see how the circuit tracks sudden external stimuli. Furthermore, to tackle complex sequential switching scenarios, we wield the **Unit Step Function**. This powerful mathematical tool unifies physical mechanical switching events into elegant functional expressions, enabling us to precisely resolve multi-stage dynamic processes and capture voltage and current at every instant.
