...
> - **适用于谁** 这篇文章不是一份面向所有人的温和入门指南。它默认读者愿意主动探索、接受较高标准、能容忍前期挫败，并且对更硬核的工程和技术有兴趣。如果你现在只想先平稳入门、降低压力，那这篇文章目前可能未必适合你。
> - **注意** 由于个人职业，这份指北带有明显的系统取向，如果你励志决定偏 AI、理论等其他发展路线，仍可借鉴方法，但不必照单全收。
{: .note}
...
![miku](/assets/img/cs-guide/miku.png#w60)
...
![Workspace](/assets/img/cs-guide/workspace.jpg#w60)
...
![WSL](/assets/img/cs-guide/win.jpeg#w60)
...
> 有了**代码的版本控制**，那**环境的版本控制**呢？
>
> 很多时候你会发现：“这行代码在我电脑上跑得好好的，怎么在你那就不行了？”这通常是因为运行环境（依赖库、系统配置）不一致导致的。这时，**[Docker](https://www.docker.com/)** 便登场了。如果说 Git 是代码的“时光机”，那 Docker 就是环境的“集装箱”。它能将你的代码连同整个运行环境打包，确保在任何地方运行的结果都完全一致。而且其与前文提到的 WW 组合完美兼容，通过`Dev Containers`，彻底避免了环境混乱的噩梦。
>
> 更有趣的是，当你把这两者结合：通过 GitHub 的 **CI/CD (自动化流水线)**，每当你 `git push` 一行新代码，GitHub 就能自动帮你构建出一个 Docker 镜像并存放在 **GHCR (GitHub Container Registry)** 中。这意味着你的代码不仅被记录了，它还被自动“铸造”成了随处可用的工具。
> {: .note}
...
![ba](/assets/img/cs-guide/ba.jpg#w40)
...
![c](/assets/img/cs-guide/c.jpg#w40)
...
<details markdown="1">
  <summary><strong>有了道路，那走法呢</strong></summary>

1. 如果你是**集邮式刷项目**，很容易**又累又空**；但如果你每一段都**抓住核心问题**去啃，**收益会很高**
2. 如果你给自己**安排太满**，大概率会先 **burnout**；但如果你**边做边总结**“这门课到底在训练什么能力”，**成长会快很多**

C 之道，不会修炼“聪不聪明”，修炼的是“能不能长期忍受自己不懂”。那有什么推荐的走法？每个人的学习方法不同，我纯粹提一些建议与注意事项。

- 打地基，但限制战线: 目标不是学很多，而是建立直觉，防止后续陷入空中楼阁。
- 真实代码: 很多人只做课程，不进真实代码，不是“把 xx 源码读完”，那是幻想，而是带着问题读局部模块，做定点爆破。
- 强制加入“输出环节”: 很多人只闷头写代码，其实不够。原因不是“打造个人品牌”这么肤浅，而是输出会逼你发现自己到底哪里没懂。这一步是这几点中最**重要**的，对成长速度非常有帮助。

最后是，如何避免**半路暴毙**，谁也不想半途而废，这部分比路线本身还重要。
- 不要并行开太多坑，同时最多一个主项目一个辅阅读
- 不要把“做完项目”误当“理解系统”
- 不要只看视频不写代码
- 不要只写学院派代码不读现实派源码
- 不要只追求功能正确，不做 benchmark 和 profiling
- 不要长期没有输出总结，否则你会以为自己懂了
...
![python](/assets/img/cs-guide/python.png#w50)
...
*   选择 **Ubuntu**，可能意味着你追求**稳定与开箱即用**，是一种务实的**工程师性格**。
*   选择 **Arch Linux**，则代表了一种截然不同的哲学：**掌控、简洁、拥抱前沿**。它会劝退缺乏耐心的人，但会深深吸引那些享受从零构建、热爱掌控、并以解决问题为乐的人。
*   亦或者是，直接选择 **Docker**，享受一项目一镜像的**五星级酒店拎包入住**，是一种飘逸的**旅人性格**。

![arch](/assets/img/cs-guide/arch.png)
...
![java](/assets/img/cs-guide/java.webp)
...
![flying](/assets/img/cs-guide/flying.png)
